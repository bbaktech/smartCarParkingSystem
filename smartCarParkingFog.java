...
public class smartCarParkingFog {
	...
	static int numOfCamerasPerArea1=1;
	...
	public static void main(String[] args) {
		...
		try {
			...
			String appId = "SCPA"; // identifier of the application
			...
			moduleMapping.addModuleToDevice("record_on_cloud", "cloud"); // fixing instances of User Interface module in the Cloud
			moduleMapping.addModuleToDevice("transfer_to_cloud", "proxy-server"); // fixing instances of User Interface module in the Cloud
	...
	private static void createFogDevices(int userId, String appId) {
		...
		Actuator SSDStore = new Actuator("store1", userId, appId,"STORE");
		actuators.add(SSDStore);
		SSDStore.setGatewayDeviceId(cloud.getId());
		SSDStore.setLatency(5.0);  // latency of connection between PTZ Control and the parent Smart Camera is 1 ms
	...
	private static FogDevice addArea(String id, int userId, String appId, int parentId){
		...
		//add for one LED for one area 		
		Actuator ptz = new Actuator("ptz-"+id, userId, appId, "PTZ_CONTROL");
		actuators.add(ptz);
		ptz.setGatewayDeviceId(router.getId());
	...
	private static Application createApplication(String appId, int userId){
		...
		application.addAppModule("transfer_to_cloud", 10);
		application.addAppModule("record_on_cloud", 10);

		/*
		 * Connecting the application modules (vertices) in the application model (directed graph) with edges
		 */
		application.addAppEdge("CAMERA", "picture-capture", 1000, 500, 
				"CAMERA", Tuple.UP, AppEdge.SENSOR); // adding edge from CAMERA (sensor) to Motion Detector module carrying tuples of type CAMERA

		application.addAppEdge("picture-capture", "slot-detector",1000, 500, 
				"slots",Tuple.UP, AppEdge.MODULE);	

		// adding edge from Slot Detector to PTZ CONTROL (actuator)
		application.addAppEdge("slot-detector", "PTZ_CONTROL", 100,	28,100,
				"PTZ_PARAMS",Tuple.ACTUATOR, AppEdge.ACTUATOR);

		application.addAppEdge("slot-detector", "transfer_to_cloud",  28, 100, 
				"slots1",Tuple.UP, AppEdge.MODULE);	
		
		application.addAppEdge("transfer_to_cloud", "record_on_cloud",  28, 100, 
				"slots2",Tuple.UP, AppEdge.MODULE);	

		application.addAppEdge("record_on_cloud", "STORE", 5, 28, 100,
				"STORE11",Tuple.ACTUATOR, AppEdge.ACTUATOR);	

		application.addTupleMapping("picture-capture", "CAMERA", "slots",
				new FractionalSelectivity(1.0));	
		
		application.addTupleMapping("slot-detector", "slots","PTZ_PARAMS",
				new FractionalSelectivity(0.1));

		application.addTupleMapping("slot-detector", "slots","slots1",
				new FractionalSelectivity(0.1));
		
		application.addTupleMapping("transfer_to_cloud", "slots1","slots2",
				new FractionalSelectivity(0.05));
		
		application.addTupleMapping("record_on_cloud", "slots2","STORE11",
				new FractionalSelectivity(0.05));
